...
<div class="promotion-search">
    ...
    <div class="card card-outline card-info">
        <div class="card-header">
            ...
            <?= Html::button('<i class="icofont icofont-plus-circle"></i> Create Promotion', ['value' => Url::to('promotion/create') , 'class' => 'btn btn-success btn-sm' , 'id'=>'btn-modal-create']) ?>
            </div>
            <!-- /.card-tools -->
        </div>
        <!-- /.card-header -->
        <div class="card-body">
            <div class="form-group-sm row">
                <div class="col-sm-3">
                    <label class="col-form-label-sm">Promotion Name (TH)</label>
                    <?= $form->field($model, 'promotion_name_th')->textInput(['value' => (!empty($search['Promotion']['promotion_name_th'])) ? $search['Promotion']['promotion_name_th'] : '' ])?>
                </div>
                <div class="col-sm-3">
                    <label class="col-form-label-sm">Promotion Name (EN)</label>
                    <?= $form->field($model, 'promotion_name_en')->textInput(['value' => (!empty($search['Promotion']['promotion_name_en'])) ? $search['Promotion']['promotion_name_en'] : '' ])?>
                </div>
                <div class="col-sm-3">
