...
<div class="jobs-search">
    ...
    <div class="card card-outline card-info">
        <div class="card-header">
            ...
            <?= Html::button('<i class="icofont icofont-plus-circle"></i> Create Jobs', ['value' => Url::to('jobs/create') , 'class' => 'btn btn-success btn-sm' , 'id'=>'btn-modal-create']) ?>
            </div>
            <!-- /.card-tools -->
        </div>
        <!-- /.card-header -->
        <div class="card-body">
            <div class="form-group-sm row">
                <div class="col-sm-3">
                    <label class="col-form-label-sm">Jobs Name (TH)</label>
                    <?= $form->field($model, 'jobs_name_th')->textInput(['value' => (!empty($search['Jobs']['jobs_name_th'])) ? $search['Jobs']['jobs_name_th'] : '' ])?>
                </div>
                <div class="col-sm-3">
                    <label class="col-form-label-sm">Jobs Name (EN)</label>
                    <?= $form->field($model, 'jobs_name_en')->textInput(['value' => (!empty($search['Jobs']['jobs_name_en'])) ? $search['Jobs']['jobs_name_en'] : '' ])?>
                </div>
                <div class="col-sm-3">
